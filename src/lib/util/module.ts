...
const mkdir = (dirPath, dirname = undefined) => {
...
const copyDir = (template, page, dist) => {
  ...
  const splitPageNameArr = page
    .match(/[A-Z][a-z]*/g)
    .map(item => item.toLowerCase());

  urls.push({
    url: `/${splitPageNameArr.join("-")}`
  });

  const bufferedContent = fd
    .toString()
    .replace("namespace: ''", `namespace: "${page.toLowerCase()}"`);

  exec(`cp -r ${template} ${target}`, (err, stdout, stderr) => {
    if (err) {
...
export { deleteAll, mkdir, copyDir };
